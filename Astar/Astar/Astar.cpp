...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		break;
	case WM_SIZE: 
		// wParam : 윈도우창의 변경상황 여부, lParam(HIWORD : 윈도우 높이, LOWORD : 윈도우 너비)
		{
			GetClientRect(hWnd, &rect);
			//P.Delete_Map();
			//P.Init(rect);
	...
	case WM_KEYDOWN:
		...
		break;
	case WM_RBUTTONDOWN:
		{// 오른쪽 마우스 키 누르고 있으면 벽생성
			flag = true;
			P.CreateBlock(LOWORD(lParam), HIWORD(lParam), d_flag);
			InvalidateRect(hWnd, NULL, false);
		}
		break;
	case WM_RBUTTONUP:
	...
	case WM_LBUTTONDOWN:
		{
			state++;
			P.Start_to_Target(LOWORD(lParam), HIWORD(lParam), state);
			InvalidateRect(hWnd, NULL, false);
		}
		break;
    case WM_COMMAND:
