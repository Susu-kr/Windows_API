...
BOOL CALLBACK ChatServerProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_INITDIALOG:
	{
		...
		btnOpen = GetDlgItem(hWnd, IDC_BUTTON_OPEN);
		btnClose = GetDlgItem(hWnd, IDC_BUTTON_CLOSE);
	...
	case WM_ASYNC:
	{
		...
		case FD_ACCEPT:
		{
			...
			SendMessage(msgList, LB_ADDSTRING, 0, (LPARAM)_T("유저 입장"));
		}
		break;
		...
		case FD_CLOSE:
		{
			...
			SendMessage(msgList, LB_ADDSTRING, 0, (LPARAM)_T("유저 퇴장"));
		}
		break;
		}
	}
	...
	case WM_COMMAND:
	{
		...
		case IDC_BUTTON_OPEN:
		{
			...
			{
				SendMessage(msgList, LB_ADDSTRING, 0, (LPARAM)_T("서버 오픈"));
		...
		case IDC_BUTTON_CLOSE:
		{
			...
			SendMessage(msgList, LB_ADDSTRING, 0, (LPARAM)_T("서버 종료"));
